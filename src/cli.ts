...
import { addProfile, getProfile, loadConfig, Platform, platforms, polyu, stateHome, useProfile } from './config.js';
...
const program = new Command().name('lms').description('PolyU Canvas & Blackboard CLI · natural-language queries').version('0.2.0').option('-p, --profile <id>', 'PolyU account profile').option('--json', 'Explicit machine-readable JSON output (default for structured commands)');
...
program.command('init').description('Create the PolyU profile').requiredOption('--preset <name>', 'Currently: polyu').action(async o => {
  if (o.preset !== 'polyu') throw new LmsError('BAD_INPUT', 'Only the PolyU profile is available in this release.');
  const existing = (await loadConfig()).profiles.find(p => p.id === 'polyu'); out(existing ?? await addProfile(polyu));
});
const profilesCommand = program.command('profiles').description('View the PolyU account configuration');
...
const auth = program.command('auth').description('PolyU school authorization');
auth.command('login').option('--platform <name>', 'canvas or blackboard; omit for all').action(async o => {
  ...
  process.stderr.write('已打开登录窗口，请完成 PolyU 登录。完成后会自动继续。\n');
...
program.command('doctor').description('Local diagnostics, without reading or printing credentials').action(async () => {
  ...
  out({ version: '0.2.0', node: process.version, os: process.platform, arch: process.arch, stateHome: stateHome(), profiles: (await loadConfig()).profiles.map(p => p.id), authorizationRuntimeInstalled: electron, nativeKeyringModuleLoads: keyringModule, readOnly: true, note: 'Module loading is not an OS keychain or school login test. Use auth login and auth status --live for acceptance.' });
});
